  ...
  useEffect(() => {
    const checkVesselsInGeofences = () => {
      ...
      vessels.forEach((vessel) => {
        ...
        // Get only the date format function
        ...
        polygonGeofences.forEach((geofence) => {
          ...
          if (isInside) {
            // Check if vessel is not already marked as Inside
            if (vesselTableData[vessel.name]?.status !== 'Inside') {
              const geofenceInsideTime = getCurrentDate(); // Get only the date
              updatedVesselEntries[vessel.name] = {
                entryTime: vesselTableData[vessel.name]?.entryTime || geofenceInsideTime, // Keep existing entry time
                geofence: geofence.geofenceName,
                status: 'Inside',
                exitTime: null
              };
              isInsideAnyGeofence = true;
              updateGeofenceInDB(vessel.name, 'Inside', geofenceInsideTime);
            }
          }
        });
        ...
        polylineGeofences.forEach((geofence) => {
          ...
          if (isNearPolyline) {
            // Check if vessel is not already marked as Near
            if (!updatedVesselEntries[vessel.name] || !updatedVesselEntries[vessel.name].status.includes('Near')) {
              updatedVesselEntries[vessel.name] = {
                entryTime: vesselTableData[vessel.name]?.entryTime || getCurrentDate(), // Get only the date
                geofence: geofence.geofenceName,
                status: `Near ${Math.round(distanceToPolyline)} meters`,
                exitTime: null
              };
              isInsideAnyGeofence = true;
              updateGeofenceInDB(vessel.name, 'Near', getCurrentDate());
            }
          }
        });
        ...
        if (!isInsideAnyGeofence && (vesselTableData[vessel.name]?.status === 'Inside' || vesselTableData[vessel.name]?.status.includes('Near'))) {
          updatedVesselEntries[vessel.name] = {
            status: 'Outside',
            exitTime: getCurrentDate() // Get only the date
          };
        }
      });
